...
public:
    ...
    void newsearch(unsigned int x, unsigned int y, unsigned int num);
    void border_check(unsigned int x, unsigned int y,  int & size);
...
/*void Matrix :: search(unsigned int x, unsigned int y, unsigned int num){
        ...
        if(findzero(x, y)){
            ...
            for ( int tempsize = size - 1; tempsize > 0 ; tempsize--) {
...
void Matrix :: newsearch(unsigned int x, unsigned int y, unsigned int num){
        ...
        if(findzero(x, y)){
            ...
            int tempsize;
            if((size - 1) % 2 == 0) tempsize = size - 2;
            else tempsize = size - 1;

            for (; tempsize > 0 ; tempsize -= 2){
...
void Matrix :: border_check(unsigned int x, unsigned int y,  int & size){
